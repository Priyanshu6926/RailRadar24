...
import { ArrowLeft, Share2, Check, MapPin, CloudSun, Mountain, AlertCircle, TrendingUp, Train, Bell } from 'lucide-react';
...
import { HistoricalAnalytics } from '@/features/analytics/HistoricalAnalytics';
import { TerrainPanel } from '@/features/terrain/TerrainPanel';
import { CoachComposition } from '@/features/coach/CoachComposition';
import { PlatformFinder } from '@/features/platform/PlatformFinder';
import { DelayAlerts } from '@/features/alerts/DelayAlerts';
...
export default function TrainJourneyPage({ params }: { params: { id: string } }) {
  ...
  const handleShare = () => {
    ...
    if (typeof navigator.share === 'function') {
      navigator
        .share({ title: `RailRadar24 – ${journey?.name || `Train #${trainId}`}`, url: shareUrl })
        .catch(() => {
  ...
  return (
    <div className="space-y-4 py-2">
      ...
      {/* ─── Tab Selector ─── */}
      <div className="flex items-center gap-1 rounded-2xl glass-panel p-1.5 shadow-glass overflow-x-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-6">
          ...
          {activeTab === 'history' && (
            <HistoricalAnalytics trainId={journey.trainId} />
          )}
          {activeTab === 'coach' && (
            <CoachComposition trainId={journey.trainId} trainName={journey.name} />
          )}

          {/* Platform Finder + Report Card — shown on map and history tabs */}
          {(activeTab === 'map' || activeTab === 'history') && (
            <div className="space-y-4">
              <PlatformFinder journey={journey} />
              <JourneyReportCard journey={journey} />
            </div>
          )}

          {/* Delay Alerts for mobile (hidden on desktop where it's in top bar) */}
          {activeTab === 'map' && (
            <div className="sm:hidden glass-panel rounded-2xl p-4 space-y-3">
              <div className="flex items-center gap-2">
                <Bell className="h-4 w-4 text-rail-blue" />
                <h4 className="text-sm font-bold text-slate-900 dark:text-white">Delay Alerts</h4>
              </div>
              <DelayAlerts journey={journey} />
            </div>
          )}
